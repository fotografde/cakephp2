...
class ShellDispatcher
{
    ...
    protected function _initConstants()
    {
        ...
        if (!defined('CAKE_CORE_INCLUDE_PATH')) {
            define('CAKE_CORE_INCLUDE_PATH', dirname(__FILE__, 3));
    ...
    protected function _bootstrap()
    {
        ...
        if (!defined('TMP') && !is_dir(APP . 'tmp')) {
            define(
                'TMP',
                CAKE_CORE_INCLUDE_PATH . DS . 'Cake' . DS . 'Console' . DS . 'Templates' . DS . 'skel' . DS . 'tmp' . DS
            );
        }
    ...
    public function dispatch()
    {
        ...
        $private = str_starts_with((string)$command, '_') && method_exists($Shell, $command);
    ...
    protected function _getShell($shell)
    {
        ...
        if (!class_exists($class)) {
            throw new MissingShellException(['class' => $class]);
        }
        $Shell = new $class();
        $Shell->plugin = trim((string)$plugin, '.');
        return $Shell;
    }
    ...
    public function parseParams($args)
    {
        ...
        $defaults = ['app' => 'app', 'root' => dirname(__FILE__, 4), 'working' => null, 'webroot' => 'webroot'];
        $params = [...$defaults, ...array_intersect_key($this->params, $defaults)];
        $isWin = false;
        foreach ($defaults as $default => $value) {
            if (str_contains((string)$params[$default], '\\')) {
                $isWin = true;
                break;
            }
        }
