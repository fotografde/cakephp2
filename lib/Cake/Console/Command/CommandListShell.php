...
class CommandListShell extends AppShell
{
    ...
    public function main()
    {
        if (empty($this->params['xml'])) {
            ...
            $this->out(" -working: " . rtrim((string)APP, DS));
            $this->out(" -root: " . rtrim((string)ROOT, DS));
            $this->out(" -core: " . rtrim((string)CORE_PATH, DS));
            $this->out(" -webroot: " . rtrim((string)WWW_ROOT, DS));
            $this->out("");
            ...
            $this->out(
                __d('cake_console', "Example: %s or %s", '-app relative/path/to/myapp', '-app /absolute/path/to/myapp'),
                2
            );
    ...
    protected function _asText($shellList)
    {
        ...
        $this->out(
            __d('cake_console', "To get help on a specific command, type <info>cake shell_name --help</info>"),
            2
        );
    }
    ...
    public function getOptionParser()
    {
        ...
        )->addOption('sort', ['help' => __d('cake_console', 'Does nothing (deprecated)'), 'boolean' => true]
        )->addOption('xml', ['help' => __d('cake_console', 'Get the listing as XML.'), 'boolean' => true]);
