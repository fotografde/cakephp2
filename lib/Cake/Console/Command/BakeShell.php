...
class BakeShell extends AppShell
{
        ...
        switch ($classToBake) {
            ...
            default:
                $this->out(
                    __d(
                        'cake_console',
                        'You have made an invalid selection. Please choose a type of class to Bake by entering D, M, V, F, T, or C.'
                    )
                );
        }
        $this->hr();
    ...
    public function getOptionParser()
    {
        ...
        $parser->description(
        ...
        )->addSubcommand('all', ['help' => __d('cake_console', 'Bake a complete MVC. optional <name> of a Model')]
        )->addSubcommand(
            'project',
            [
                'help' => __d(
                    'cake_console',
                    'Bake a new app folder in the path supplied or in current directory if no path is specified'
                ),
            ...
            [
                'help' => __d(
                    'cake_console',
                    'Bake a new plugin folder in the path supplied or in current directory if no path is specified.'
                ),
            ...
            'model',
            ['help' => __d('cake_console', 'Bake a model.'), 'parser' => $this->Model->getOptionParser()]
        )->addSubcommand(
            'view',
            ['help' => __d('cake_console', 'Bake views for controllers.'), 'parser' => $this->View->getOptionParser()]
        )->addSubcommand(
            'controller',
            ['help' => __d('cake_console', 'Bake a controller.'), 'parser' => $this->Controller->getOptionParser()]
        )->addSubcommand(
            'fixture',
            ['help' => __d('cake_console', 'Bake a fixture.'), 'parser' => $this->Fixture->getOptionParser()]
        )->addSubcommand(
            'test',
            ['help' => __d('cake_console', 'Bake a unit test.'), 'parser' => $this->Test->getOptionParser()]
        )->addOption(
            'connection',
            [
