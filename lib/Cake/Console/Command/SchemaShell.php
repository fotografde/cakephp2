...
class SchemaShell extends AppShell
{
    ...
    public function startup()
    {
        ...
        if (!str_contains((string)$this->params['file'], '.php')) {
    ...
    public function generate()
    {
        ...
        if (!$snapshot && file_exists($this->Schema->path . DS . $this->params['file'])) {
            ...
            $result = strtolower((string)$this->in($prompt, ['o', 's', 'q'], 's'));
            if ($result === 'q') {
                return $this->_stop();
            }
            if ($result === 'o') {
    ...
    public function dump()
    {
        ...
        if ($write) {
            if (!str_contains((string)$write, '.sql')) {
                $write .= '.sql';
            }
            if (str_contains((string)$write, DS)) {
    ...
    protected function _loadSchema()
    {
        ...
        $options = ['name' => $name, 'plugin' => $plugin, 'connection' => $this->params['connection']];
        if (!empty($this->params['snapshot'])) {
            $fileName = basename((string)$this->Schema->file, '.php');
        ...
        if (!$Schema) {
            $this->err(
                __d('cake_console', '<error>Error</error>: The chosen schema could not be loaded. Attempted to load:')
            );
    ...
    public function getOptionParser()
    {
        ...
        $plugin = ['short' => 'p', 'help' => __d('cake_console', 'The plugin to use.')];
        $connection = [
            'short' => 'c',
            'help' => __d('cake_console', 'Set the db config to use.'),
            'default' => 'default',
        ];
        $path = ['help' => __d('cake_console', 'Path to read and write schema.php'), 'default' => CONFIG . 'Schema'];
        $file = ['help' => __d('cake_console', 'File name to read and write.')];
        $name = [
            'help' => __d(
                'cake_console',
        ...
        $snapshot = ['short' => 's', 'help' => __d('cake_console', 'Snapshot number to use/make.')];
        $models = ['short' => 'm', 'help' => __d('cake_console', 'Specify models as comma separated list.')];
        $dry = [
            'help' => __d(
                'cake_console',
        ...
        $write = ['help' => __d('cake_console', 'Write the dumped SQL to a file.')];
        $exclude = ['help' => __d('cake_console', 'Tables to exclude as comma separated list.')];
        $yes = [
            'short' => 'y',
        ...
        $parser->description(
            ...
            [
                ...
                'parser' => [
                    ...
                    'arguments' => ['snapshot' => ['help' => __d('cake_console', 'Generate a snapshot.')]],
                ],
            ]
        )->addSubcommand(
            'dump',
            ...
            [
                ...
                'parser' => [
                    ...
                    'args' => [
                        'name' => ['help' => __d('cake_console', 'Name of schema to use.')],
                        'table' => ['help' => __d('cake_console', 'Only create the specified table.')],
                    ],
                ],
            ]
        )->addSubcommand(
