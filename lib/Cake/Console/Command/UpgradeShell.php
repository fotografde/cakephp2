...
class UpgradeShell extends AppShell
{
    ...
    public function locations()
    {
        ...
        $defaultOptions = ['recursive' => true, 'checkFolder' => true, 'regex' => '@class (\S*) .*(\s|\v)*{@i'];
    ...
    public function helpers()
    {
        ...
        $patterns = [];
        App::build(['View/Helper' => App::core('View/Helper')], App::APPEND);
        ...
        foreach ($helpers as $helper) {
            ...
            $patterns[] = ["\${$oldHelper} to \$this->{$helper}", "/\\\${$oldHelper}->/", "\\\$this->{$helper}->"];
        }
    ...
    public function i18n()
    {
        ...
        $patterns = [
            ['<?php __*(*) to <?php echo __*(*)', '/<\?php\s*(__[a-z]*\(.*?\))/', '<?php echo \1'],
            ['<?php __*(*, true) to <?php echo __*()', '/<\?php\s*(__[a-z]*\(.*?)(,\s*true)(\))/', '<?php echo \1\3'],
    ...
    public function basics()
    {
        ...
        $patterns = [
            ['a(*) -> array(*)', '/\ba\((.*)\)/', 'array(\1)'],
            ['e(*) -> echo *', '/\be\((.*)\)/', 'echo \1'],
            ['ife(*, *, *) -> !empty(*) ? * : *', '/ife\((.*), (.*), (.*)\)/', '!empty(\1) ? \2 : \3'],
            ['r(*, *, *) -> str_replace(*, *, *)', '/\br\(/', 'str_replace('],
            ['up(*) -> strtoupper(*)', '/\bup\(/', 'strtoupper('],
            ['low(*) -> strtolower(*)', '/\blow\(/', 'strtolower('],
            ['getMicrotime() -> microtime(true)', '/getMicrotime\(\)/', 'microtime(true)'],
        ];
        $this->_filesRegexpUpdate($patterns);
    }
    ...
    public function request()
    {
        ...
        $patterns = [
            ['$this->data -> $this->request->data', '/(\$this->data\b(?!\())/', '$this->request->data'],
            ['$this->params -> $this->request->params', '/(\$this->params\b(?!\())/', '$this->request->params'],
            ['$this->webroot -> $this->request->webroot', '/(\$this->webroot\b(?!\())/', '$this->request->webroot'],
            ['$this->base -> $this->request->base', '/(\$this->base\b(?!\())/', '$this->request->base'],
            ['$this->here -> $this->request->here', '/(\$this->here\b(?!\())/', '$this->request->here'],
            ['$this->action -> $this->request->action', '/(\$this->action\b(?!\())/', '$this->request->action'],
            [
    ...
    public function constants()
    {
        ...
        $patterns = [
            ["LIBS -> CAKE", '/\bLIBS\b/', 'CAKE'],
            ["CONFIGS -> APP . 'Config' . DS", '/\bCONFIGS\b/', 'APP . \'Config\' . DS'],
            ["CONTROLLERS -> APP . 'Controller' . DS", '/\bCONTROLLERS\b/', 'APP . \'Controller\' . DS'],
            [
                "COMPONENTS -> APP . 'Controller' . DS . 'Component' . DS",
                '/\bCOMPONENTS\b/',
                'APP . \'Controller\' . DS . \'Component\'',
            ],
            ["MODELS -> APP . 'Model' . DS", '/\bMODELS\b/', 'APP . \'Model\' . DS'],
            [
                "BEHAVIORS -> APP . 'Model' . DS . 'Behavior' . DS",
                '/\bBEHAVIORS\b/',
                'APP . \'Model\' . DS . \'Behavior\' . DS',
            ],
            ["VIEWS -> APP . 'View' . DS", '/\bVIEWS\b/', 'APP . \'View\' . DS'],
            ["HELPERS -> APP . 'View' . DS . 'Helper' . DS", '/\bHELPERS\b/', 'APP . \'View\' . DS . \'Helper\' . DS'],
            [
            ...
            ["CONSOLE_LIBS -> CAKE . 'Console' . DS", '/\bCONSOLE_LIBS\b/', 'CAKE . \'Console\' . DS'],
            ["CAKE_TESTS_LIB -> CAKE . 'TestSuite' . DS", '/\bCAKE_TESTS_LIB\b/', 'CAKE . \'TestSuite\' . DS'],
            ["CAKE_TESTS -> CAKE . 'Test' . DS", '/\bCAKE_TESTS\b/', 'CAKE . \'Test\' . DS'],
        ];
        $this->_filesRegexpUpdate($patterns);
    }
    ...
    public function exceptions()
    {
        ...
        if (!empty($this->params['plugin'])) {
            $pluginPath = CakePlugin::path($this->params['plugin']);
            $this->_paths = [$pluginPath . 'controllers' . DS, $pluginPath . 'controllers' . DS . 'components' . DS];
        }
        $patterns = [
            [
    ...
    public function getOptionParser()
    {
        ...
        $subcommandParser = [
            'options' => [
                ...
                'ext' => [
                    ...
                    'help' => __d(
                        'cake_console',
                        'The extension(s) to search. A pipe delimited list, or a preg_match compatible subpattern'
                    ),
        ...
        $parser->description(
            ...
            'all',
            ['help' => __d('cake_console', 'Run all upgrade commands.'), 'parser' => $subcommandParser]
        )->addSubcommand(
            'tests',
            [
            ...
            'locations',
            ['help' => __d('cake_console', 'Move files and folders to their new homes.'), 'parser' => $subcommandParser]
        )->addSubcommand(
            'i18n',
            ['help' => __d('cake_console', 'Update the i18n translation method calls.'), 'parser' => $subcommandParser]
        )->addSubcommand(
            'helpers',
            ['help' => __d('cake_console', 'Update calls to helpers.'), 'parser' => $subcommandParser]
        )->addSubcommand(
            'basics',
            [
            ...
            'constants',
            ['help' => __d('cake_console', "Replace Obsolete constants"), 'parser' => $subcommandParser]
        )->addSubcommand(
            'controller_redirects',
            ['help' => __d('cake_console', 'Return early on controller redirect calls.'), 'parser' => $subcommandParser]
        )->addSubcommand(
            'components',
            [
            ...
            'exceptions',
            ['help' => __d('cake_console', 'Replace use of cakeError with exceptions.'), 'parser' => $subcommandParser]
        );
