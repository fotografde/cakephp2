...
class ConsoleShell extends AppShell
{
     ...
     * @return void
     */
    protected function _bind(mixed $command)
    {
        ...
        if ($this->_isValidModel($modelA) && $this->_isValidModel($modelB) && in_array(
                $association,
                $this->associations
            )) {
     ...
     * @return void
     */
    protected function _unbind(mixed $command)
    {
        preg_match($this->_methodPatterns[__FUNCTION__], (string)$command, $tmp);
        ...
        if ($this->_isValidModel($modelA) && $this->_isValidModel($modelB) && in_array(
            ...
            $this->out(
                __d(
                    'cake_console',
                    "Please verify you are using valid models, valid current association, and valid association types"
                )
            );
        }
    }
     ...
     * @return void
     */
    protected function _find(mixed $command)
    {
     ...
     * @return void
     */
    protected function _save(mixed $command)
    {
     ...
     * @return void
     */
    protected function _columns(mixed $command)
    {
        preg_match($this->_methodPatterns[__FUNCTION__], (string)$command, $tmp);
    ...
    protected function _routesReload()
    {
        if (!$this->_loadRoutes()) {
            return $this->err(
                __d(
                    'cake_console',
                    "There was an error loading the routes config. Please check that the file exists and is free of parse errors."
                )
            );
        }
     ...
     * @return void
     */
    protected function _routeToString(mixed $command)
    {
        preg_match($this->_methodPatterns[__FUNCTION__], (string)$command, $tmp);
     ...
     * @return void
     */
    protected function _routeToArray(mixed $command)
    {
        preg_match($this->_methodPatterns[__FUNCTION__], (string)$command, $tmp);
