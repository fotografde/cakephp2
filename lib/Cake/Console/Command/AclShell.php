...
class AclShell extends AppShell
{
    ...
    public function startup()
    {
        ...
        if (!in_array($class, ['DbAcl', 'DB_ACL']) && !is_subclass_of($class, 'DbAcl')) {
            ...
            $out .= __d(
                    'cake_console',
                    'Please change your core config to reflect your decision to use DbAcl before attempting to use this script'
                ) . "\n";
            $out .= "--------------------------------------------------\n";
        ...
        if ($this->command) {
            if (!config('database')) {
                $this->out(
                    __d('cake_console', 'Your database configuration was not found. Take a moment to create one.')
                );
                $this->args = null;
    ...
    public function create()
    {
        ...
        if (is_string($data) && $data !== '/') {
            ...
            $this->error(
                __d('cake_console', '/ can not be used as an alias!') . __d(
                    'cake_console',
                    "	/ is the root, please supply a sub alias"
                )
            );
        }
    ...
    public function delete()
    {
        ...
        if ($this->Acl->{$class}->find('all', ['conditions' => $identifier])) {
            if (!$this->Acl->{$class}->deleteAll($identifier)) {
                $this->error(
                    __d('cake_console', 'Node Not Deleted. ') . __d(
                        'cake_console',
                        'There was an error deleting the %s.',
                        $class
                    ) . "\n"
                );
            }
            ...
            $this->error(
                __d('cake_console', 'Node Not Deleted. ') . __d(
                    'cake_console',
                    'There was an error deleting the %s. Node does not exist.',
                    $class
                ) . "\n"
            );
        }
    ...
    public function setParent()
    {
        ...
        if (!$this->Acl->{$class}->save($data)) {
            $this->out(
                __d(
                    'cake_console',
                    'Error in setting new parent. Please make sure the parent node exists, and is not a descendant of the node specified.'
                )
            );
        } else {
    ...
    public function view()
    {
        ...
        if (isset($this->args[1])) {
            ...
            $topNode = $this->Acl->{$class}->find(
                'first',
                ['conditions' => [$class . '.id' => $this->_getNodeId($class, $identity)]]
            );

            $nodes = $this->Acl->{$class}->find(
                'all',
                [
                    'conditions' => [
        ...
        if (empty($nodes)) {
            if (isset($this->args[1])) {
                $this->error(
                    __d('cake_console', '%s not found', $this->args[1]),
                    __d('cake_console', 'No tree returned.')
                );
            } elseif (isset($this->args[0])) {
                $this->error(
                    __d('cake_console', '%s not found', $this->args[0]),
                    __d('cake_console', 'No tree returned.')
                );
            }
        }
        ...
        $parser->description(
            ...
            [
                ...
                'parser' => [
                    ...
                    'arguments' => [
                        'type' => $type,
                        'node' => ['help' => __d('cake_console', 'The node identifier to delete.'), 'required' => true],
                    ],
                ],
            ]
        )->addSubcommand(
            ...
            [
                ...
                'parser' => [
                    'description' => __d(
                        'cake_console',
                        'Moves the ACL object specified by <node> beneath <parent>'
                    ),
                    'arguments' => [
                        'type' => $type,
                        'node' => ['help' => __d('cake_console', 'The node to move'), 'required' => true],
                        'parent' => ['help' => __d('cake_console', 'The new parent for <node>.'), 'required' => true],
                    ],
                ],
            ]
        )->addSubcommand(
            ...
            [
                ...
                'parser' => [
                    'description' => [
                        __d(
                            'cake_console',
                            "Returns the path to the ACL object specified by <node>."
                        ),
                        __d(
                            'cake_console',
                            "This command is useful in determining the inheritance of permissions for a certain object in the tree."
                        ),
                    ],
                    'arguments' => [
                        'type' => $type,
                        'node' => ['help' => __d('cake_console', 'The node to get the path of'), 'required' => true],
                    ],
                ],
            ]
        )->addSubcommand(
            ...
            [
                ...
                'parser' => [
                    'description' => [__d('cake_console', 'Use this command to check ACL permissions.')],
                    'arguments' => [
