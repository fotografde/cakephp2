...
class TestTask extends BakeTask
{
        ...
        if ($type) {
            ...
            if (!isset($this->classTypes[$type])) {
                $this->error(
                    __d(
                        'cake_console',
                        'Incorrect type provided. Please choose one of %s',
                        implode(', ', array_keys($this->classTypes))
                    )
                );
            }
        } else {
    ...
    public function getObjectType()
    {
        ...
        $keys[] = 'q';
        $selection = $this->in(
            __d('cake_console', 'Enter the type of object to bake a test for or (q)uit'),
            $keys,
            'q'
        );
        if ($selection === 'q') {
    ...
    public function getClassName($objectType)
    {
        ...
        while (empty($selection)) {
            $selection = $this->in(
                __d('cake_console', 'Choose an existing class, or enter the name of a class that does not exist')
            );
    ...
    {
        $proceed = $this->in(
            __d('cake_console', 'Bake could not detect fixtures, would you like to add some?'),
            ['y', 'n'],
            'n'
        );
        $fixtures = [];
    ...
    public function getOptionParser()
    {
        ...
        $parser->description(
            ...
            [
                'help' => __d(
                    'cake_console',
                    'Type of class to bake, can be any of the following: controller, model, helper, component or behavior.'
                ),
                'choices' => [
                    'Controller',
                    'controller',
        ...
        )->addArgument('name', ['help' => __d('cake_console', 'An existing class to bake tests for.')])->addOption(
            'theme',
            ['short' => 't', 'help' => __d('cake_console', 'Theme to use when baking code.')]
        )->addOption(
            'plugin',
            ['short' => 'p', 'help' => __d('cake_console', 'CamelCased name of the plugin to bake tests for.')]
        )->addOption(
            'force',
