...
class FixtureTask extends BakeTask
{
    ...
    public function getOptionParser()
    {
        ...
        $parser->description(
            ...
            'plugin',
            ['help' => __d('cake_console', 'CamelCased name of the plugin to bake fixtures for.'), 'short' => 'p']
        )->addOption(
            'schema',
            [
            ...
            'help' => __d(
                'cake_console',
                'Used with --count and <name>/all commands to pull [n] records from the live tables, ' .
    ...
    public function importOptions($modelName)
    {
        ...
        if (!empty($this->params['schema'])) {
            ...
            $doSchema = $this->in(
                __d('cake_console', 'Would you like to import schema for this fixture?'),
                ['y', 'n'],
                'n'
            );
            if ($doSchema === 'y') {
        ...
        if (!empty($this->params['records'])) {
            ...
            $doRecords = $this->in(
                __d('cake_console', 'Would you like to use record importing for this fixture?'),
                ['y', 'n'],
                'n'
            );
            if ($doRecords === 'y') {
    ...
    {
        $schema = trim((string)$this->_Schema->generateTable('f', $tableInfo), "\n");
    ...
    protected function _getRecordsFromTable($modelName, $useTable = null)
    {
        ...
        if ($this->interactive) {
            ...
            $recordsFound = $modelObject->find('count', ['conditions' => $condition, 'recursive' => -1]);
        ...
        $records = $modelObject->find('all', ['conditions' => $condition, 'recursive' => -1, 'limit' => $recordCount]);
