...
class PluginTask extends AppShell
{
    ...
    protected function _interactive($plugin = null)
    {
        ...
        if (!$this->bake($plugin)) {
            $this->error(
                __d('cake_console', "An error occurred trying to bake: %s in %s", $plugin, $this->path . $plugin)
            );
        }
    }
    ...
    public function getOptionParser()
    {
        ...
        )->addArgument('name', ['help' => __d('cake_console', 'CamelCased name of the plugin to create.')]);
