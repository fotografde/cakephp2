...
class ExtractTask extends AppShell
{
    ...
    protected function _getPaths()
    {
        ...
        while (true) {
            ...
            if (strtoupper((string)$response) === 'Q') {
                ...
                $this->err(
                    __d('cake_console', '<warning>No directories selected.</warning> Please choose a directory.')
                );
    ...
    public function execute()
    {
        ...
        if (isset($this->params['extract-core'])) {
            ...
            $response = $this->in(
                __d('cake_console', 'Would you like to extract the messages from the CakePHP core?'),
                ['y', 'n'],
                'n'
            );
        ...
        if ($this->_extractCore) {
            $this->_paths[] = CAKE;
            $this->_exclude = array_merge($this->_exclude, [CAKE . 'Test', CAKE . 'Console' . DS . 'Templates']);
        }
        ...
        if (isset($this->params['merge'])) {
            ...
            $response = $this->in(
                __d(
                    'cake_console',
                    'Would you like to merge all domain and category strings into the default.pot file?'
                ), ['y', 'n'],
                'n'
            );
    ...
    protected function _addTranslation($category, $domain, $msgid, $details = [])
    {
        ...
        if (empty($this->_translations[$category][$domain][$msgid][$context])) {
            $this->_translations[$category][$domain][$msgid][$context] = ['msgid_plural' => false];
        }
    ...
    public function getOptionParser()
    {
        ...
        $parser->description(
        ...
        )->addOption('app', ['help' => __d('cake_console', 'Directory where your application is located.')])->addOption(
            'paths',
            ...
            'no-location',
            ['boolean' => true, 'default' => false, 'help' => __d('cake_console', 'Do not write lines with locations')]
        )->addOption('output', ['help' => __d('cake_console', 'Full path to output directory.')])->addOption(
            'files',
            ...
            [
                ...
                'help' => __d(
                    'cake_console',
                    'Ignores all files in plugins if this command is run inside from the same app directory.'
                ),
            ]
        )->addOption(
            'plugin',
            [
                'help' => __d(
                    'cake_console',
                    'Extracts tokens only from the plugin specified and puts the result in the plugin\'s Locale directory.'
                ),
            ]
            ...
            [
                'help' => __d(
                    'cake_console',
                    'If set to a value, the localization domain to be used for model validation messages.'
                ),
            ]
        )->addOption('exclude', [
            'help' => __d(
            ...
            'extract-core',
            ['help' => __d('cake_console', 'Extract messages from the CakePHP core libs.'), 'choices' => ['yes', 'no']]
        );
    ...
    protected function _processValidationRules($field, $rules, $file, $domain, $category = 'LC_MESSAGES')
    {
        ...
        foreach ($rules as $rule => $validateProp) {
            ...
            if ($msgid) {
                ...
                $details = ['file' => $file, 'line' => 'validation for field ' . $field];
    ...
    protected function _buildFiles()
    {
        ...
        usort($paths, fn($a, $b) => strlen((string)$b) - strlen((string)$a));
            ...
            foreach ($domains as $domain => $sentences) {
                ...
                while ($overwriteAll === false && $File->exists() && strtoupper((string)$response) !== 'Y') {
                    ...
                    if (strtoupper((string)$response) === 'N') {
                        ...
                        while (!$response) {
                            $response = $this->in(
                                __d('cake_console', "What would you like to name this file?"),
                                null,
                                'new_' . $filename
                            );
