...
class ControllerTask extends BakeTask
{
    ...
    public function all()
    {
        ...
        if (!$controllersCreated) {
            $this->out(
                __d('cake_console', 'No Controllers were baked, Models need to exist before Controllers can be baked.')
            );
        }
    }
    ...
    public function bakeActions($controllerName, $admin = null, $wannaUseSession = true)
    {
        ...
        if (!class_exists($modelImport)) {
            $this->err(
                __d('cake_console', 'You must have a model for this class to build basic methods. Please try again.')
            );
            return $this->_stop();
        }
    ...
    public function bake($controllerName, $actions = '', $helpers = null, $components = null)
    {
        ...
        $this->Template->set(
            ['plugin' => $this->plugin, 'pluginPath' => empty($this->plugin) ? '' : $this->plugin . '.']
        );
    ...
    public function getOptionParser()
    {
        ...
        $parser->description(
            ...
            [
                'help' => __d(
                    'cake_console',
                    'Name of the controller to bake. Can use Plugin.name to bake controllers into plugins.'
                ),
            ]
        )->addOption(
            'public',
        ...
        )->addOption('plugin', ['short' => 'p', 'help' => __d('cake_console', 'Plugin to bake the controller into.')]
        )->addOption(
            'connection',
            ['short' => 'c', 'help' => __d('cake_console', 'The connection the controller\'s model is on.')]
        )->addOption('theme', ['short' => 't', 'help' => __d('cake_console', 'Theme to use when baking code.')]
        )->addOption(
            'force',
            ['short' => 'f', 'help' => __d('cake_console', 'Force overwriting existing files without prompting.')]
        )->addSubcommand('all', ['help' => __d('cake_console', 'Bake all controllers with CRUD methods.')])->epilog(
