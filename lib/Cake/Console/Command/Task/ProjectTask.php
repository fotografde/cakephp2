...
class ProjectTask extends AppShell
{
    ...
    public function execute()
    {
        ...
        if (isset($this->args[0])) {
            ...
            if (empty($appContents)) {
                $suggestedPath = rtrim((string)APP, DS);
            } else {
        ...
        if ($this->bake($project)) {
            ...
            if ($this->securitySalt($path) === true) {
                ...
                $this->err(
                    __d(
                        'cake_console',
                        'Unable to generate random hash for \'Security.salt\', you should change it in %s',
                        CONFIG . 'core.php'
                    )
                );
                $success = false;
            ...
            if ($this->securityCipherSeed($path) === true) {
                ...
                $this->err(
                    __d(
                        'cake_console',
                        'Unable to generate random seed for \'Security.cipherSeed\', you should change it in %s',
                        CONFIG . 'core.php'
                    )
                );
                $success = false;
            ...
            if ($this->cakeOnIncludePath()) {
                $this->out(
                    __d(
                        'cake_console',
                        '<info>CakePHP is on your `include_path`. CAKE_CORE_INCLUDE_PATH will be set, but commented out.</info>'
                    )
                );
            } else {
                $this->out(
                    __d(
                        'cake_console',
                        '<warning>CakePHP is not on your `include_path`, CAKE_CORE_INCLUDE_PATH will be hard coded.</warning>'
                    )
                );
            ...
            if ($success) {
                $this->out(
                    __d(
                        'cake_console',
                        ' * CAKE_CORE_INCLUDE_PATH set to %s in %s',
                ...
                $this->err(
                    __d(
                        'cake_console',
                        'Unable to set CAKE_CORE_INCLUDE_PATH, you should change it in %s',
                        $path . 'webroot' . DS . 'index.php'
                    )
                );
                $success = false;
            }
            if ($success && $hardCode) {
                $this->out(
                    __d(
                        'cake_console',
                        '   * <warning>Remember to check these values after moving to production server</warning>'
                    )
                );
            }
    ...
    public function consolePath($path)
    {
        ...
        if (preg_match('/(__CAKE_PATH__)/', (string)$contents, $match)) {
            $root = str_starts_with((string)CAKE_CORE_INCLUDE_PATH, '/') ? " DS . '" : "'";
    ...
    public function securitySalt($path)
    {
        ...
        if (preg_match('/([\s]*Configure::write\(\'Security.salt\',[\s\'A-z0-9]*\);)/', (string)$contents, $match)) {
            ...
            $result = str_replace(
                $match[0],
                "\t" . 'Configure::write(\'Security.salt\', \'' . $string . '\');',
                (string)$contents
            );
            if ($File->write($result)) {
                return true;
    ...
    public function securityCipherSeed($path)
    {
        ...
        if (preg_match(
            ...
            App::uses('Security', 'Utility');
            $string = substr(bin2hex((string)Security::generateAuthKey()), 0, 30);
            $result = str_replace(
                $match[0],
                "\t" . 'Configure::write(\'Security.cipherSeed\', \'' . $string . '\');',
                (string)$contents
            );
            if ($File->write($result)) {
                return true;
    ...
    public function getOptionParser()
    {
        ...
        )->addOption('theme', ['short' => 't', 'help' => __d('cake_console', 'Theme to use when baking code.')]
        )->addOption('skel', [
            'default' => current(App::core('Console')) . 'Templates' . DS . 'skel',
            'help' => __d(
                'cake_console',
                'The directory layout to use for the new application skeleton.' .
