...
class DbConfigTask extends AppShell
{
        ...
        while (!$done) {
            ...
            while (!$name) {
                ...
                if (preg_match('/[^a-z0-9_]/i', (string)$name)) {
                    ...
                    $name = '';
                    $this->out(
                        __d('cake_console', 'The name must start with an unaccented latin character or an underscore')
                    );
                }
            }

            $datasource = $this->in(__d('cake_console', 'Datasource:'), ['Mysql', 'Sqlite'], 'Mysql');
            ...
            while (!$password && !$blankPassword) {
                ...
                if (!$password) {
                    $blank = $this->in(
                        __d('cake_console', 'The password you supplied was empty. Use an empty password?'), ['y', 'n'],
                        'n'
                    );
                    if ($blank === 'y') {
                        $blankPassword = true;
    ...
    public function bake($configs)
    {
        ...
        foreach ($configs as $config) {
            ...
            if (!str_contains((string)$datasource, 'Database/')) {
