...
class ModelTask extends BakeTask
{
        ...
        while ($anotherValidator === 'y') {
            ...
            if ($this->interactive === true) {
                ...
                if (in_array($choice, $alreadyChosen)) {
                    $this->out(
                        __d('cake_console', "You have already chosen that validation rule,\nplease choose again")
                    );
                    continue;
                }
    ...
    public function doAssociations($model)
    {
        ...
        $associations = ['belongsTo' => [], 'hasMany' => [], 'hasOne' => [], 'hasAndBelongsToMany' => []];
    ...
    public function findBelongsTo(Model $model, $associations)
    {
        ...
        foreach ($fieldNames as $fieldName) {
            $offset = str_ends_with((string)$fieldName, '_id');
    ...
    public function findHasOneAndMany(Model $model, $associations)
    {
        ...
        foreach ($this->_tables as $otherTable) {
            ...
            $pattern = '/_' . preg_quote((string)$model->table, '/') . '|' . preg_quote(
                    (string)$model->table,
                    '/'
                ) . '_/';
    ...
    public function doMoreAssociations(Model $model, $associations)
    {
        ...
        while (strtolower((string)$wannaDoMoreAssoc) === 'y') {
            ...
            if (!isset($foreignKey)) {
                $foreignKey = $this->in(
                    __d('cake_console', 'What is the foreignKey? Specify your own.'),
                    null,
                    $suggestedForeignKey
                );
            }
            if ($assocType === 3) {
                $associationForeignKey = $this->in(
                    __d('cake_console', 'What is the associationForeignKey?'),
                    null,
                    $this->_modelKey($model->name)
                );
    ...
    public function bake($name, $data = [])
    {
        ...
        $this->Template->set(['plugin' => $this->plugin, 'pluginPath' => $pluginPath]);
    ...
    public function getOptionParser()
    {
        ...
        )->addOption('plugin', ['short' => 'p', 'help' => __d('cake_console', 'Plugin to bake the model into.')]
        )->addOption('theme', ['short' => 't', 'help' => __d('cake_console', 'Theme to use when baking code.')]
        )->addOption(
            'connection',
            ['short' => 'c', 'help' => __d('cake_console', 'The connection the model table is on.')]
        )->addOption(
            'force',
