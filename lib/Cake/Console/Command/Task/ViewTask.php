...
class ViewTask extends BakeTask
{
        ...
        foreach ($methods as $i => $method) {
            if ($adminRoute && !empty($this->params['admin'])) {
                if ($scaffoldActions) {
                    ...
                    continue;
                } elseif (!str_contains((string)$method, (string)$adminRoute)) {
                    unset($methods[$i]);
                }
            }
    ...
    protected function _interactive()
    {
        ...
        $wannaDoAdmin = $this->in(
            __d('cake_console', "Would you like to create the views for admin routing?"),
            ['y', 'n'],
            'n'
        );
    ...
    public function customAction()
    {
        ...
        $this->out(
            __d(
                'cake_console',
                'Path:            %s',
                $this->getPath() . $this->controllerName . DS . Inflector::underscore($action) . ".ctp"
            )
        );
        $this->hr();
    ...
    public function getTemplate($action)
    {
        ...
        foreach ((array)$prefixes as $prefix) {
            if (str_contains($template, (string)$prefix)) {
    ...
    public function getOptionParser()
    {
        ...
        $parser->description(
            ...
            [
                'help' => __d(
                    'cake_console',
                    'Name of the controller views to bake. Can be Plugin.name as a shortcut for plugin baking.'
                ),
            ]
        )->addArgument(
            'action',
        ...
        )->addOption('plugin', ['short' => 'p', 'help' => __d('cake_console', 'Plugin to bake the view into.')]
        )->addOption(
            'admin',
            [
            ...
            'connection',
            ['short' => 'c', 'help' => __d('cake_console', 'The connection the connected model is on.')]
        )->addOption(
            'force',
            ...
            [
                'help' => __d(
                    'cake_console',
                    'Bake all CRUD action views for all controllers. Requires models and controllers to exist.'
                ),
            ]
        )->epilog(
        ...
        foreach ($keys as $type) {
            foreach ($model->{$type} as $assocKey => $assocData) {
                ...
                $associations[$type][$assocKey]['controller'] = Inflector::pluralize(
                    Inflector::underscore($modelClass)
                );
