...
class FileEngine extends CacheEngine
{
     ...
     */
    public function decrement($key, $offset = 1): never
    {
     ...
     * @throws CacheException
     */
    public function increment($key, $offset = 1): never
    {
    ...
    public function clearGroup($group)
    {
        ...
        foreach ($contents as $object) {
            $containsGroup = str_contains((string)$object->getPathName(), DS . $group . DS);
            $hasPrefix = true;
            if (strlen((string)$this->settings['prefix']) !== 0) {
                $hasPrefix = str_starts_with((string)$object->getBaseName(), (string)$this->settings['prefix']);
            }
