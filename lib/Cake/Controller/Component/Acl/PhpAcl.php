...
class PhpAcl extends CakeObject implements AclInterface
{
    ...
    {
        $this->options = ['policy' => static::DENY, 'config' => CONFIG . 'acl.php'];
    }
...
class PhpAro
{
     ...
     * @var string
     */
    final public const DEFAULT_ROLE = 'Role/default';
     ...
     */
    public $map = ['User' => 'User/username', 'Role' => 'User/role'];
    ...
    public function resolve($aro)
    {
        foreach ($this->map as $aroGroup => $map) {
            [$model, $field] = explode('/', (string)$map, 2);
            $mapped = '';
