...
class CookieComponent extends Component
{
     ...
     */
    public function write($key, mixed $value = null, $encrypt = true, $expires = null)
    {
    ...
    {
        $availableTypes = ['cipher', 'rijndael', 'aes'];
        if (!in_array($type, $availableTypes)) {
            trigger_error(
                __d('cake_dev', 'You must use cipher, rijndael or aes for cookie encryption type'),
                E_USER_WARNING
            );
            $type = 'cipher';
        }
    ...
    protected function _encrypt($value)
    {
        ...
        return $prefix . base64_encode((string)$cipher);
    }
    ...
    protected function _explode($string)
    {
        ...
        if ($first === '{' || $first === '[') {
            $ret = json_decode($string, true, 512, JSON_THROW_ON_ERROR);
            return $ret ?? $string;
        }
        $array = [];
