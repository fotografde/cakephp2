...
class SecurityComponent extends Component
{
     ...
     */
    final public const DEFAULT_EXCEPTION_MESSAGE = 'The request has been black-holed';
    ...
    protected function _hashParts(Controller $controller)
    {
        ...
        return [$controller->request->here(), serialize($fieldList), $unlocked, Configure::read('Security.salt')];
    }
    ...
    {
        $messages = [];
        $expectedParts = json_decode(
            urldecode((string)$controller->request->data['_Token']['debug']),
            true,
            512,
            JSON_THROW_ON_ERROR
        );
    ...
    public function generateToken(CakeRequest $request)
    {
        ...
        $authKey = hash('sha512', (string)Security::randomBytes(16), false);
        $token = [
            'key' => $authKey,
        ...
        $request->params['_Token'] = ['key' => $token['key'], 'unlockedFields' => $token['unlockedFields']];
        return true;
    }
