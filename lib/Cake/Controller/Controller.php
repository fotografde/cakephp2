...
class Controller extends CakeObject implements CakeEventListener
{
    ...
    {
        return match ($name) {
            'base', 'here', 'webroot', 'data' => $this->request->{$name},
            'action' => $this->request->params['action'] ?? '',
            'params' => $this->request,
            'paginate' => $this->Components->load('Paginator')->settings,
        ...
        try {
            ...
            if ($this->_isPrivateAction($method, $request)) {
                throw new PrivateActionException(
                    ['controller' => $this->name . "Controller", 'action' => $request->params['action']]
                );
            }
    ...
    protected function _isPrivateAction(ReflectionMethod $method, CakeRequest $request)
    {
        ...
        if (!$privateAction && !empty($prefixes)) {
            if (empty($request->params['prefix']) && strpos((string)$request->params['action'], '_') > 0) {
                [$prefix] = explode('_', (string)$request->params['action']);
     ...
     */
    protected function _parseBeforeRedirect(mixed $response, $url, $status, $exit)
    {
     ...
     */
    public function set($one, mixed $two = null)
    {
        if (is_array($one)) {
            if (is_array($two)) {
    ...
    public function postConditions($data = [], $op = null, $bool = 'AND', $exclusive = false)
    {
        ...
        foreach ($data as $model => $fields) {
            if (preg_match($allowedChars, (string)$model)) {
