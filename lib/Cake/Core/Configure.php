...
class Configure
{
     ...
     * @var array
     */
    protected static $_values = ['debug' => 0];
    ...
    public static function bootstrap($boot = true)
    {
        if ($boot) {
            ...
            $exception = ['handler' => 'ErrorHandler::handleException'];
            $error = ['handler' => 'ErrorHandler::handleError', 'level' => E_ALL & ~E_DEPRECATED];
            if (PHP_SAPI === 'cli') {
     ...
     */
    public static function write($config, mixed $value = null)
    {
        if (!is_array($config)) {
            $config = [$config => $value];
        }
